...
**Sessions are stateless.** They don't automatically continue from where they left off. They fire when the user sends a message or a scheduled event occurs. "Next session" is not a guaranteed event. It's a conditional that depends entirely on external triggers.
...
**Standup-class failure**: Recurring observation agents (standups, health checks) are session-mode agents that end after posting their report. They cannot schedule follow-on work unless the instructions explicitly include that step. The standup "promises" work that the standup session cannot deliver.

**End-of-session deferral**: Agents near context limits identify important actions but defer them because the session is ending. Without a scheduled follow-up, the deferral is permanent.

## Context

This pattern applies to any agent operating across multiple sessions, especially:
- Agents running scheduled recurring jobs (standups, health checks, digests)
...
2. **If time doesn't permit**: Schedule a follow-up agent with the action as the instruction

**There is no third path.** "Will handle later" without a concrete trigger is equivalent to dropping the item.

**Scheduling as the execution artifact:**
```
"I'll handle that next session"  →  zero mechanism, 0% completion
"Scheduled: follow-up agent at [T+2h]: [full action instructions]"  →  guaranteed trigger, 100% completion rate
```

A scheduled agent is the difference between an intention and an artifact. The artifact cannot be forgotten; it fires regardless of whether the user re-raises the item.

**Standup instruction discipline**: Every recurring observation agent must include a final step in its instructions that creates follow-up agents for any actionable findings it cannot resolve inline. Without this step, the standup creates work but no execution path for that work.

Example standup final step:
```
FINAL STEP: For each actionable item that cannot be resolved in this session,
schedule a follow-up agent with:
- Full context of what to do and why
- Target start time: [T+15min for self-decidable, T+30min for complex]
- Output routing matching where the work belongs
```

**Instruction completeness for handoff agents**: The scheduled agent's instructions ARE the context for the next session. Include everything needed to execute without the current session's history: what to do, why, key file paths, acceptance criteria. A thin instruction set produces a confused agent that re-investigates work already done.
...
**Context-limit deferral recovery**: In an analysis of 20 sessions that hit context limits, 8 produced end-of-session statements like "will continue this next time." Of the 8: 1 was followed up organically (user re-raised). 7 were permanently dropped. After applying the follow-through rule (schedule an agent before ending the session), 9 subsequent context-limit sessions produced scheduled follow-ups — 9/9 were completed by their scheduled time.

**Instruction quality effect on scheduled agents**: In 12 scheduled agents created with thin instructions ("handle the open PR review"), 5 (42%) produced agents that re-investigated context already assembled in the originating session, consuming their full budget on investigation without acting. In 8 agents created with full-context instructions (file paths, PR numbers, specific issues, acceptance criteria), 0 re-investigated. The instructions are the context; thin instructions waste agent cycles.
...
**Cost**: Creating a scheduled agent adds ~1–2 minutes per deferred item and requires assembling context into coherent instructions. Under time pressure (near context limit, session ending), this investment competes with other end-of-session work.

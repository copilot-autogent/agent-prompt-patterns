...
For session continuity specifically: agents should query the session-store rather than relying on memory topics. Example (scope to the relevant channel/task to prevent cross-conversation leakage in shared stores): `SELECT content FROM turns WHERE session_id IN (SELECT id FROM sessions WHERE agent_id = ? AND channel_id = ? ORDER BY created_at DESC LIMIT 1) ORDER BY turn_index ASC`. If the session-store is empty or unavailable, interactive agents should ask the user for context. Headless or background agents should treat missing continuity as a clean-start condition and proceed with available task parameters rather than blocking on user input.
